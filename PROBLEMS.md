...
5. **Cross-family 10-seed panel-composition robustness extension** — protocol SHA256-locked before any additional panel seed was run. An import-safe executable runner and deterministic unit tests are now present, but the confirmatory ten-seed experiment remains intentionally unexecuted. Its eventual result is a new artifact and does not modify the existing single-seed result.
...
7. ~~**Mapper augmentation under full node membership** (audit blocker B4)~~ — **RESOLVED (v1.5.0, 2026-06-15).** The robustness harness was executed (LFS payload available). Two findings. (a) A biased-panel **deduplication defect** was fixed: the 30%-overlapping Mapper cover let boundary proteins recur across node member lists and `rng.choice(replace=False)` removes positions, not values, so the biased arm held fewer than 500 unique positives. After `np.unique`, the committed-method rescue is −0.0080 (was +0.0018); CI still includes zero, H1 still rejected. (b) Under the *published per-arm stratification*, full node membership appears to support H1 (rescue +0.084 multi-node, +0.172 single-node), but this is a **stratification artifact** — the cluster-concentrated biased panel enlarges its own distant stratum (biased/uniform `n_dist` ratio up to 1.16), redefining "distant" to include easier points. A **common-stratification control** (committed as `controlled_rescue_*` in `mapper_augmentation_robustness.json`), scoring both arms on the same distant queries, collapses the rescue to ≈0 (−0.004 multi-node, −0.006 single-node; the truncated arm's controlled rescue is +0.016), every CI spanning zero — consistent with the committed truncated result under its own stratification (−0.008, H1 also rejected). The Attempt-4 null is **robust to full node membership**; the Paper 2 "four failed rescues" conclusion stands. The prose/code discrepancy (single top node vs multi-node accumulation) is both tested (`full_single_node`, `full_multinode`) and noted in the Paper 2 erratum. See `docs/MAPPER_AUGMENTATION_ROBUSTNESS.md`.
...
- ~~Zenodo deposit with DOI~~ — COMPLETED v1.5.3 (DOI `10.5281/zenodo.20143142`; see `CITATION.cff`).
