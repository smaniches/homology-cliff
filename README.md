...
We characterize the cliff across a 3,000-cell pre-registered factorial and show it is not a stratification artifact via a matching 3,000-cell full-pool permutation null. The original Pfam partition found 20/20 evaluable distant false alarms cross-family at seed 20260410. A subsequently SHA256-locked ten-seed confirmation, run without changing the model, metric, threshold, neighbor count, annotation source, or family rule, found cross-family predominance in every panel seed: median per-seed cross-family fraction 1.000, mean 0.994, range 0.962-1.000, with all ten seeds nonzero evaluable. This satisfies the preregistered strong robustness criterion and materially weakens a purely within-family panel-coverage explanation, while not claiming that within-family errors are absent. We pre-register and reject four rescue hypotheses: Mahalanobis whitening, Fisher-Rao whitening, a stratified metric cascade, and topologically-biased panel augmentation. One rescue survives all tests: a cheap supervised linear projection of the embedding space, fit only on the panel in under five seconds of CPU time, which wins pooled $F_1$ in 16 of 18 factorial groups (12 of 12 at t12 and t30; the two misses are at the smallest t6/8M scale at $k=5$) and improves distant-stratum $F_1$ by 47% relative at the largest scale. The deployment consequence is direct: **apply the projection, and even then, route every distant-stratum positive hit to human review**.
...
- **Reviewers and journal editors**: all claims are traceable to committed evidence via `MANIFEST.sha256.json`. Experimental hypotheses were pre-registered before execution; three registrations are SHA256-locked and CI-verified, while other committed registrations are not hash-enforced. See `PROBLEMS.md` for the self-audited list of errors caught during authoring and items deferred.
...
| 5 | **Cross-Family Predominance and Mapper Topology** | Original seed: 20/20 evaluable distant false alarms cross-family. Preregistered ten-seed confirmation: cross-family predominance in every seed; median 1.000, mean 0.994, range 0.962-1.000; strong robustness criterion passed. | `papers/05_cross_family_and_mapper/paper.pdf` |
...
| Cross-family analysis, original seed | `data/results_summaries/cross_family_partition.json` | small | Per-case Pfam sets for all 41 distant false alarms at seed 20260410 |
| Cross-family panel-seed confirmation | `data/results_summaries/cross_family_partition_10seed.json` | 242 KB | Ten fixed panel seeds, full distant-FP detail, seed-level aggregate, accession histories, preregistered decision |
...
| `data/prereg/PRE_REGISTRATION_CROSS_FAMILY_10SEED_v1.md` | `204953efeb098f70…` | Ten fixed panel seeds, seed-level aggregation, no pooled binomial interval, strong robustness decision rule |
...
The original main, full-null, cascade, and Fisher registrations were committed before their corresponding experiments. The later cross-family ten-seed registration was separately SHA256-locked before any additional panel seed was run. Three registrations are now SHA256-locked and re-verified in CI: the main cliff (`139f6012...`), full-pool null (`f3864d09...`), and cross-family ten-seed extension (`204953ef...`). The main cliff registration is additionally enforced at runtime by its harnesses; the cross-family runner verifies the frozen inputs and requires exact reproduction of seed 20260410 before proceeding. Cascade and Fisher-Rao remain committed without a CI hash lock.
...
- Cross-family panel-composition robustness is now confirmed across the ten fixed seeds, but only at ESM-2 t30, R=1000, k=25, threshold 0.90, this dataset, and the Pfam ontology. External models, family ontologies, and third-party replication remain open.
...
```
{
  ...
  "headline_numbers": {
    ...
    "cross_family_seed_20260410": "20/20",
    "cross_family_10seed_median": 1.000,
    "cross_family_10seed_mean": 0.994,
    "cross_family_10seed_range": "0.962-1.000",
    "cross_family_10seed_strong_robustness": true,
